...
find $LOCATION -name "* *" | rename 's/ //g' &>/dev/null
...
#サーバーディレクトリの登録
if [ -z $SERVER ] || [ $ChangeConditions -eq 1 ] || [ ! -f $SERVER/scripts/remote-control/runKernel.sh ]; then

	serverdirinfo=(`find $LOCATION -name runKernel.sh | sed 's@/scripts/remote-control/runKernel.sh@@g'`)
	...
	if [ ! ${#serverdirinfo[@]} -eq 1 ]; then
		...
		for i in ${serverdirinfo[@]}; do
			...
			serverdirinfo[$count]=`echo $i | sed 's@/@ @g' | awk '{print $NF}'`"+@+"$i
		...
		echo
		echo "上のリストからサーバーを選択してください(0を入力するとデフォルトを選択します)。"
		...
		do
			...
			if [ ! -z `expr "$servernumber" : '\([0-9][0-9]*\)'` ] && [ 0 -lt $servernumber ] && [ $servernumber -le $line ]; then
				...
				if [ -f $SERVER/scripts/remote-control/runKernel.sh ]; then
...
if [ -z $SRC ] || [ $ChangeConditions -eq 1 ] || [ ! -f $SRC/library/rescue/adf/adf-core.jar ]; then
	...
	srcdirinfo=($(find $LOCATION -name adf-core.jar | sed 's@/library/rescue/adf/adf-core.jar@@g'))
...
if [ ! -f $SERVER/$MAP/scenario.xml ] || [ $ChangeConditions -eq 1 ] || [ -z $MAP ]; then
	...
	if [ ! ${#mapdirinfo[@]} -eq 1 ]; then
		...
		for i in ${mapdirinfo[@]}; do
			...
			printf "%s\n"  `echo $mapdir | sed "s@$SERVER/@@g" | sed 's@/map/@@g' | sed "s@$mapname/@$mapname@g"`
...
do
	...
	echo -e "\e[10;0H" #カーソルを10行目の0列目に戻す
