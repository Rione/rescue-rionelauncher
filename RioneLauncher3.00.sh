...
echo 'histry_Ver=`curl --connect-timeout 1 https://raw.githubusercontent.com/MiglyA/bash-rescue/master/histry.txt | grep "RioneLauncher-newVersion"`' >> update.sh
...
echo curl \`curl https://raw.githubusercontent.com/MiglyA/bash-rescue/master/histry.txt '| grep' "RioneLauncher-link" '| awk' \''{print $2}'\'\` '> temp' >> update.sh
...
killcommand(){
	...
	kill `ps aux | grep "$git_address/roborescue-v1.2" | awk '{print $2}'` &>/dev/null

	rm histry_date &>/dev/null
	rm update.sh &>/dev/null
	...
	if [ ! ${#mapdirinfo[@]} -eq 1 ]; then
		...
		do
			...
			if [ 0 -lt $mapnumber ] && [ $mapnumber -le $line ]; then
				...
				if [ -f $git_address/roborescue-v1.2/$map/scenario.xml ]; then
...
#瓦礫有無選択
defalutblockade=`cat $git_address/roborescue-v1.2/boot/config/collapse.cfg | grep "collapse.create-road-blockages" | awk '{print $2}'`
...
[ -z $debug ] || [ ! $((`cat RioneLauncher.sh | grep -v '^\s*#' | grep -c ""` - `grep -n '？↓' RioneLauncher.sh | sed -n 1P | sed 's/:/ /g' | awk '{print $1}'`)) -eq $debug ] && errerbreak
...
do
	...
	if [ `grep -c "Score:" server.log` -ge 1 ]; then

		echo
		echo "● シミュレーション終了！！"
		echo
		echo "◆ 最終スコアは"`cat server.log | grep "Score:" | sed -n 1P | awk '{print $2}'`"でした。"
		echo `date +%Y/%m/%d_%H:%M`　"スコア:"`cat server.log | grep "Score:" | sed -n 1P | awk '{print $2}'`　"マップ:"`echo $map | sed 's@/map@@g' | sed 's@/maps@maps@g'`　"瓦礫:"$brockademenu    >> score.log
		echo
