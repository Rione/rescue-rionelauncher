...
if [ -z $SERVER ] || [ $ChangeConditions -eq 1 ] || [ ! -f $SERVER/boot/start-comprun.sh ]; then
	...
	serverdirinfo=(`find ~/ -type d -name ".*" -prune -o -type f -name 'start-comprun.sh' -print | sed 's@/boot/start-comprun.sh@@g'`)
	...
	if [ ! ${#serverdirinfo[@]} -eq 1 ]; then
		...
		do	
			...
			printf "%s\n" `echo $serverdir | sed "s@/$servername@@g"`
...
if [ -z $SRC ] || [ $ChangeConditions -eq 1 ] || [ ! -f $SRC/library/rescue/adf/adf-core.jar ]; then
	...
	srcdirinfo=(`find ~/ -type d -name ".*" -prune -o -type f -name 'adf-core.jar' -print | sed 's@/library/rescue/adf/adf-core.jar@@g'`)
...
if [ ! -f $SERVER/$MAP/scenario.xml ] || [ $ChangeConditions -eq 1 ] || [ -z $MAP ]; then
	...
	if [ ! ${#mapdirinfo[@]} -eq 1 ]; then
		...
		for i in ${mapdirinfo[@]}; do
			...
			printf "%s\n"  `echo $mapdir | sed "s@$SERVER/@@g" | sed 's@/map/@@g' | sed "s@$mapname/@$mapname@g" | sed "s@/$mapname@@g"`
